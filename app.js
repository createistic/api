...
const {
  ...
  AssertionClaims,
  Address,
} = require("@gruposantander/rp-client-typescript").Model;
const {
  VerifiedIdClient,
  InitiateAuthorizeRequestBuilder,
  TokenRequestBuilder,
} = require("@gruposantander/rp-client-typescript").Client;
...
const wellKnown =
  ...
  "https://live.iamid.io/.well-known/openid-configuration";
...
const staticsFolderRelativePath =
  process.env.STATICS_FOLDER || "/../dist/digital-id-example-frontend";
...
app.get("/initiate-authorize", async (req, res) => {
  ...
  claims
    ...
    .withEssential(true)
    .withPurpose(
      "Please share the email address that you wish for our teams to contact you on."
    );
  claims
    .phoneNumber()
    .withEssential(true)
    .withPurpose(
      "Please share the phone number that you wish for our teams to contact you on."
    );
  ...
  assertionClaims
    ...
    .withEssential(true)
    .withPurpose(
      "We want to verify that we have your correct first name, so that we can address any correspondence with you, and between yourself and any future sprint team organisers correctly."
    );
  assertionClaims
    .familyName()
    ...
    .withEssential(true)
    .withPurpose(
      "We want to verify that we have your correct surname, so that we can address any correspondence with you, and between yourself and any future sprint team organisers correctly."
    );
  assertionClaims
    .title()
    .equal(userDetails.title)
    .withPurpose(
      "We want to verify your title is correct, so that we can address any correspondence with you, and between yourself and any future sprint team organisers correctly in a professional manner."
    );
  assertionClaims
    .address()
    .withAssertion(
      Address.streetAddress().eq(userDetails.address.street_address)
    )
    ...
    .withPurpose(
      "We want to verify your address is correct, so that we can address any correspondence with you, and between yourself and any future sprint team organisers correctly in a professional manner. We also need to ensure that you have a place of residence within the UK."
    );
  ...
  try {
    ...
    console.log("fail 141");
    res.status(500).json({
      error: e,
      error_description: "Unable to create client instance - unset proxies",
    });
    return;
  }
  ...
  try {
    const request = new InitiateAuthorizeRequestBuilder()
      ...
      .withClaims(claims)
      .withPurpose(
        "We want to check your details are correct before allowing you to formally accept any job offers."
      )
      .build();
...
app.post("/verified", async (req, res) => {
  try {
    ...
    res.status(400).json({
      error: "Not valid value for verified",
      message: "Provide a query param with a boolean eg. ?value=true",
    });
  }
});
...
function addSharedData(tokenObject) {
  ...
  if (tokenObject.phone_number)
    userDetails.phone_number = tokenObject.phone_number;
}
